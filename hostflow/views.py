...
def send_email_otp(request):
    if request.method == "POST":
        try:
            ...
            request.session['email'], request.session['email_otp'] = email, otp
            request.session['email_verified'] = False
            send_mail('HostFlow OTP', f'Your OTP is {otp}', settings.EMAIL_HOST_USER, [email])
            return JsonResponse({'status': 'success'})
        except Exception: return JsonResponse({'status': 'fail'})
